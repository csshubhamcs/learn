 ...
 * everyday condition - bad input, a missing row, a duplicate, an expired token - must never
 * come back as 500. A 500 here should mean "our code has a bug or a dependency is down",
 * nothing else.
 */
@Slf4j
...
public class GlobalExceptionHandler {
    ...
    // internal text to the caller. The two input errors that used to rely on that handler -
    // an unknown sortBy and an unknown sortDir - now throw InvalidRequestException instead.

    // A query/path parameter that Spring cannot convert to its target type - e.g.
    // ?status=NOT_A_REAL_STATUS against the admin listing's UserStatus parameter - is a
    // client input error, not a server fault, and without this handler it would otherwise
    // fall through to handleUnexpected and answer with a 500.
    ...
    private ResponseEntity<ApiError> build(
        ...
        ApiError body = new ApiError(
                ...
                message,
                pathOf(request),
                // "traceId" is the key Micrometer Tracing's MDC instrumentation populates once
                // distributed tracing is wired up (see the micrometer-tracing-bridge-otel
                // dependency); reading it here rather than a service-local id we mint
                // ourselves means this field starts working automatically the moment tracing
                // is enabled, with no change to the API contract. Until then it is null - a
                // service-local correlation id (a previous RequestIdFilter) was deliberately
                // not reintroduced, since it only correlates log lines within this one
                // service and would just be replaced by real tracing shortly anyway.
                MDC.get("traceId"),
